...
const userList = (props: any) => {
    ...
    if(Object.keys(Usuarios).length<=1){
        if(Object.keys(Usuarios).length===0){
            dataT = [{nombre:"example" ,materno:"last", email:"example@live.com", deleted:false, inserver:true},
            {nombre:"ejemplo", materno:"last", email:"ejemplo@live.com", deleted:false, inserver:true}];
        }else{
            dataT = [Usuarios[0],
            {nombre:"ejemplo", materno:"last", email:"ejemplo@live.com", deleted:false, inserver:true}];
        };
      ...
      onRowSelectionChange:(dat:any,cell:any)=>{
          ...
          onRowsDelete:(ro:{data:[]},lookup:{})=>{
              ro.data.map((dato:{dataIndex:any})=>{
                  ...
                  let regD:any ={id:dataT[dato.dataIndex].id,nombre:dataT[dato.dataIndex].nombre};
    ...
    return (
        <GridContainer>
            <GridItem xs={12} sm={12} md={12}>
                <Card>
                    <CardHeader color="$38">
                        <h4 >Listado de Usuarios</h4><ModalUsuario create={oncreate} update={usuario} upd={onupd} />                       
                            <MUIDataTable
                            title={"Usuarios"}
                            data={dataT}
                            columns={columns}
